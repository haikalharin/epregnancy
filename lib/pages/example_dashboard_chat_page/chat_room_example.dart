...
import '../../data/firebase/event/event_chat_room_example.dart';
...
class ChatRoomExample extends StatefulWidget {
  final Map<String, dynamic> arguments;

  const ChatRoomExample({ Key? key, required this.arguments}) : super(key: key);

  @override
  _ChatRoomExampleState createState() => _ChatRoomExampleState();
}

class _ChatRoomExampleState extends State<ChatRoomExample> with WidgetsBindingObserver {
  ...
  void getMyPerson() async {
    ...
    EventChatRoomExample.setMeInRoom(_myPerson!.uid!, widget.arguments["room"].uid!);
  ...
  void sendMessage(String type, String message) async {
    ...
    if (message != '') {
      ...
      bool personInRoom = await EventChatRoomExample.checkIsPersonInRoom(
        myUid: _myPerson!.uid,
      ...
      // Sender Room
      bool isSenderRoomExist = await EventChatRoomExample.checkRoomIsExist(
        isSender: true,
        myUid: _myPerson!.uid,
        personUid: widget.arguments["room"].uid,
      );
      if (isSenderRoomExist) {
        EventChatRoomExample.updateRoom(
          isSender: true,
          myUid: _myPerson!.uid,
          personUid: widget.arguments["room"].uid,
          room: roomSender,
      ...
      // Receiver Room
      bool isReceiverRoomExist = await EventChatRoomExample.checkRoomIsExist(
        isSender: false,
        myUid: _myPerson!.uid,
        personUid: widget.arguments["room"].uid,
      );
      ...
      String token = await EventPersonExample.getPersonToken(
      ...
      if (personInRoom) {
        EventChatRoomExample.updateChatIsRead(
  ...
  void deleteSelectedMessage() {
    ...
    EventChatRoomExample.deleteMessage(
  ...
  void dispose() {
    WidgetsBinding.instance!.addObserver(this);
    EventChatRoomExample.setMeOutRoom(_myPerson!.uid!,widget.arguments["room"].uid!);
    super.dispose();
  }
