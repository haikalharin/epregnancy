...
class _NewChatRoomState extends State<NewChatRoom> {
  ...
  void pickAndCropImageGallery() async {
    ...
    if (pickedFile != null) {
      ...
      if (croppedFile != null) {
        ...
        chatMessageList?.add(ChatMessageEntity(
            mine: true,
  ...
  _initWebSocket() async {
    ...
    Future<WebSocket> futureWebSocket = WebSocket.connect(
        '${F.appFlavor == Flavor.PRODUCTION ? environment['websockets'] : F.appFlavor == Flavor.DEVELOPMENT ? devEnvironment['websockets'] : stagingEnvironment['websockets'] }${userModel.id}');
    ...
    futureWebSocket.then((WebSocket ws) {
      ...
      _webSocket.listen((d) {
        ...
        if (socketResponse['action'] == StringConstant.updateLatestChat) {
          ...
          if (fromIdPayload == toId && _toId == widget.fromId) {
            setState(() {
              chatMessageList?.add(ChatMessageEntity(
                ...
                mine: socketResponse['data']['from_id'] == widget.fromId
                    ? true
                    : false,
                imageUrl: socketResponse['data']['image_url'],
                profileImage: socketResponse['data']['from_id'] == widget.fromId
                    ? socketResponse['data']['to']['image_url']
                    : socketResponse['data']['from']['image_url'],
              ));
              isPendingChat = false;
              _scrollDown();
              _scrollController
        ...
        } else if (isPendingChat == true &&
            socketResponse['action'] == 'new-chat') {
          setState(() {
          ...
          Toast.show(
              "$toName Telah Merespon Diskusi Anda, Silahkan Jelaskan Kondisi Anda Lebih Lanjut",
              gravity: Toast.center);
        } else if (socketResponse['action'] == 'end-chat') {
          Toast.show("$toName Diskusi anda telah selesai, Terima Kasih!",
              gravity: Toast.center);
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          ...
          appBar: AppBar(
            ...
            actions: [
              chatHasEnded || isPendingChat == true
                  ? Container()
                  : Container(
                      margin:
                          EdgeInsets.only(right: 10, top: 10.h, bottom: 10.h),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        if (chatMessageList![index].mine!) {
                          if (chatMessageList![index].imagePath != null ||
                              chatMessageList![index].imageUrl != null) {
                            if (chatMessageList![index].imagePath != null) {
                              return Align(
                                  ...
                                  child: ClipRRect(
                                      ...
                                      child: Image.file(
                                        File(
                                            chatMessageList![index].imagePath!),
                                        height: 150.h,
                                        width: 150.w,
                                      )));
                            } else {
                              return Align(
                                  ...
                                  child: ClipRRect(
                                      borderRadius: BorderRadius.circular(10),
                                      child: Image.network(
                                        chatMessageList![index].imageUrl!,
                                        height: 150.h,
                                        width: 150.w,
                                      )));
                            }
                            ...
                            if (chatMessageList![index].imagePath != null) {
                              ...
                              return Align(
                                  ...
                                  child: ClipRRect(
                                      borderRadius: BorderRadius.circular(10),
                                      child: Image.network(
                                        chatMessageList![index].profileImage!,
                                        height: 150.h,
                                        width: 150.w,
                                      )));
                            }
                                        ...
                                        if (val.isNotEmpty) {
                                          if (isPendingChat == false) {
                                            ...
                                            setState(() {
                                              ...
                                              _messageEditingController.clear();
                                              _scrollDown();
                                              ChatSendRequest _chatSendRequest =
                                                  ChatSendRequest(
                                                      fromId: widget.fromId,
                                                      toId: toId,
                                                      message: val);
